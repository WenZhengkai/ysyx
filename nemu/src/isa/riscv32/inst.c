...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + (int32_t)imm);
  ...
  INSTPAT("? ?????? ????? ????? 000 ???? ? 11000 11", beq  , B, s->dnpc = (src1 == src2)? s->pc + (int32_t)imm : s->dnpc);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = ((int32_t)src1)>>((word_t)(imm - 1024)));	
...
/* have error in bit.c*/
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh    , S, Mw(src1 + (int32_t)imm, 2, src2));
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and   , R, R(rd) = src1 & src2);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ (int32_t)imm);
/* have error in bit.c*/
  INSTPAT("? ?????? ????? ????? 101 ???? ? 11000 11", bge  , B, s->dnpc = ((int32_t)src1 >= (int32_t)src2)? s->pc + (int32_t)imm : s->dnpc);

  INSTPAT("0000000 ????? ????? 101 ????? 00100 11",  srli  , I, R(rd) = ((word_t)src1)>>((word_t)imm));
  INSTPAT("? ?????? ????? ????? 111 ???? ? 11000 11", bgeu  , B, s->dnpc = ((word_t)src1 >= (word_t)src2)? s->pc + (int32_t)imm : s->dnpc);
  ...
  INSTPAT("? ?????? ????? ????? 100 ???? ? 11000 11", blt  , B, s->dnpc = ((int32_t)src1 < (int32_t)src2)? s->pc + (int32_t)imm : s->dnpc);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra	   , R, R(rd) = ((int32_t)src1)>>(src2 & 0x0000001f));
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl	   , R, R(rd) = ((word_t)src1)>>(src2 & 0x0000001f));	

  INSTPAT("? ?????? ????? ????? 110 ???? ? 11000 11", bltu  , B, s->dnpc = ((word_t)src1 < (word_t)src2)? s->pc + (int32_t)imm : s->dnpc);
